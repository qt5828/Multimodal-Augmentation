...
# GPU Number
GPU_Number = "cuda:0"
# raise Exception(torch.cuda.is_available())
...
run = wandb.init(project="MMix-COCO2",allow_val_change=True,name=args.name)
...
def do_train(trainloader,clip_model,optimizer,epoch,args,logits_scale2=None):
    print("training...")
    ...
    for batch_idx,sample in enumerate(tqdm(trainloader)):
        ...
        if epoch > -1 :
            ...
            if args.input_cutmix :
                print("Using Cutmix")
                lamb        = torch.rand(1).to(GPU_Number)
            ...
            #Make Augmented image. reverse order.
            if args.input_img :
                #lamb        = torch.rand(1).to("cuda:0")
                lamb = torch.Tensor([random.betavariate(args.betavariate,args.betavariate)]).to(GPU_Number).half()
                images_R    = torch.flip(images,dims=[0])
            ...
            if args.manifold_img :
                #lamb        = torch.rand(1).to("cuda:0").half()
                lamb = torch.Tensor([random.betavariate(args.betavariate,args.betavariate)]).to(GPU_Number).half()
            ...
            if args.manifold_txt :
                #lamb        = torch.rand(1).to("cuda:0").half()
                lamb = torch.Tensor([random.betavariate(args.betavariate,args.betavariate)]).to(GPU_Number).half()
                #lamb = torch.Tensor([random.betavariate(0.8,0.8)]).to("cuda:0").half()
            ...
            if args.manifold_all :
                ...
                lamb = torch.Tensor([random.betavariate(args.beta1,args.beta1)]).to(GPU_Number).half()
            ...
            if args.neg_mix :
                #lamb = torch.Tensor([random.betavariate(args.betavariate,args.betavariate)]).to("cuda:0").half()
                lamb = torch.Tensor([random.betavariate(args.beta2,args.beta2)]).to(GPU_Number).half()
...
def do_valid(validloader,clip_model,optimizer,args,run_calib=True,wandb_prefix="",epoch=0):
    ...
    with torch.no_grad():
        ...
        for batch_idx, sample in enumerate(validloader):
            images, text_tok = sample
            captions=text_tok
            images      = images.to(GPU_Number)
            #print(images.min() , images.max())
            images      += args.noise*torch.randn(images.shape).to(GPU_Number)
            if not args.noclip :
                text_tok    = clip.tokenize(text_tok,truncate=True).to(device)
        ...
        #     logits_per_text  = logits_per_image.T
        targets_orig        = torch.eye(logits_per_image.shape[0]).to(GPU_Number)
...
def do_valid_FGSM(validloader,clip_model,optimizer,args):
    ...
    with torch.enable_grad():
        ...
        for batch_idx, sample in enumerate(validloader):
            ...
            images      = images.to(GPU_Number)
            #images      += args.noise*torch.randn(images.shape).to("cuda:0")
            ...
            logits_per_image    = logits_per_image.type(torch.float32)
            targets_orig        = torch.eye(len(captions)).to(GPU_Number)
            final_loss          = clip_loss(logits_per_image,targets_orig)
...
class TextEncoder(nn.Module):
    ...
    def forward(self,x):
        encoded_input = self.tokenizer(x, padding=True ,truncation=True,return_tensors='pt',max_length=200).to(GPU_Number)
...
if args.dataset == "flickr" :
    trainset    = FlickerDataset('/home/wooyeon/Multimodal-Augmentation/GMMIX/dataset/flicker/flickr30k_images/flickr30k_images',transform=preprocess,perc=args.perc).filter_df("train")
    validset    = FlickerDataset('/home/wooyeon/Multimodal-Augmentation/GMMIX/dataset/flicker/flickr30k_images/flickr30k_images',transform=preprocess).filter_df("valid")
    testset     = FlickerDataset('/home/wooyeon/Multimodal-Augmentation/GMMIX/dataset/flicker/flickr30k_images/flickr30k_images',transform=preprocess).filter_df("test")
elif args.dataset == "coco" :
    trainset    = COCODataset('/home/wooyeon/Multimodal-Augmentation/GMMIX/dataset/coco/train2017',anon_path='/home/wooyeon/Multimodal-Augmentation/GMMIX/dataset/coco/annotations/captions_train2017.json',transform=preprocess,perc=args.perc)
    validset    = COCODataset('/home/wooyeon/Multimodal-Augmentation/GMMIX/dataset/coco/val2017',anon_path='/home/wooyeon/Multimodal-Augmentation/GMMIX/dataset/coco/annotations/captions_val2017.json',transform=preprocess)
    testset     = COCODataset('/home/wooyeon/Multimodal-Augmentation/GMMIX/dataset/coco/val2017',anon_path='/home/wooyeon/Multimodal-Augmentation/GMMIX/dataset/coco/annotations/captions_val2017.json',transform=preprocess)
...
cocotest            = COCODataset('/home/wooyeon/Multimodal-Augmentation/GMMIX/dataset/coco/val2017',anon_path='/home/wooyeon/Multimodal-Augmentation/GMMIX/dataset/coco/annotations/captions_val2017.json',transform=preprocess)

flickrtestloader    = DataLoader(flickrtest,    batch_size= wandb.config.bs, shuffle=False,worker_init_fn=seed_worker)
...
for epoch in range(wandb.config.epoch):
    ...
    if R1Sum > Best_R1_sum : Best_R1_sum = R1Sum
    wandb.log(I2T)
    wandb.log(T2I)
    wandb.log({"R1_Sum":R1Sum})
    
    do_train(trainloader,clip_model,optimizer,epoch=epoch,args=args)
...
torch.save({
    ...
    }, f"{run.name}_weight.pt") #just change to your preferred folder/filename
